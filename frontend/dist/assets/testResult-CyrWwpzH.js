import"./main-B2IttiPe.js";import{l as H,P as b,T as k,Q as B}from"./test-problems-D7uQa0XS.js";const x="codenergy:test:progress",M="codenergy:test:answers",T="codenergy:test:timer",C=document.getElementById("page-fade"),R=document.getElementById("result-shell");function J(){try{const s=sessionStorage.getItem(x);if(s){const a=JSON.parse(s);if(typeof(a==null?void 0:a.current)=="number")return{current:a.current,total:a.total??k}}}catch{}return{current:1,total:k}}function A(s){try{sessionStorage.setItem(x,JSON.stringify(s))}catch{}}function Q(){try{const s=sessionStorage.getItem(M);return s?JSON.parse(s):{}}catch{return{}}}const D={correct:{chip:"정답",chipClass:"result-hero__chip--correct",icon:"🎉",title:["훌륭해요!","완벽해요!","잘 풀어내셨어요!"],sub:"모든 테스트 케이스를 통과했습니다. 다음 문제도 그대로 가봐요.",enc:["이 흐름이라면 다음 문제도 무리 없겠어요.","한 번에 통과! 자신감을 가지고 가도 좋습니다.","정확하고 깔끔하네요 — 그대로 진행해주세요."]},wrong:{chip:"오답",chipClass:"result-hero__chip--wrong",icon:"💡",title:["조금만 더!","거의 다 왔어요.","다시 한 번 차분히."],sub:"일부 케이스에서 기대 출력과 달랐어요. 정답 코드와 비교해보면서 다음에 적용해보세요.",enc:["오답은 가장 좋은 학습 재료예요. 다음 문제는 더 잘 풀 수 있을 거예요.","한 발 한 발 나아가고 있어요. 흐름을 잃지 마세요.","비교 코드의 패턴을 메모해두면 다음에 큰 도움이 돼요."]},timeout:{chip:"시간 초과",chipClass:"result-hero__chip--timeout",icon:"⏳",title:["시간이 부족했어요.","아쉽게 시간 초과!","다음엔 페이스 조절!"],sub:"10분 안에 제출하지 못해 자동 제출되었습니다. 정답 코드를 살펴보고 흐름을 익혀보세요.",enc:["속도는 익숙해지면 자연스럽게 따라옵니다.","처음엔 누구나 시간이 빠듯해요 — 다음 문제로 가볼까요?","다음엔 1분 단위로 나눠서 풀어보세요."]},missing:{chip:"미제출",chipClass:"result-hero__chip--wrong",icon:"🤔",title:["제출 기록이 없어요"],sub:"이 문제를 풀고 결과 페이지로 돌아온 게 맞나요? 다시 시도하려면 처음부터 시작해주세요.",enc:["메인으로 돌아가 다시 시작할 수 있어요."]}};function O(s){return s[Math.floor(Math.random()*s.length)]}function h(s){return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function F(s,a,n){var v;const c=n[s.current-1]??n[0],i=b.find(f=>f.id===c)??b[0],t=a[i.id],d=(t==null?void 0:t.verdict)||"missing",r=D[d],g=(t==null?void 0:t.code)??"// (제출된 코드가 없습니다)",o=i.solution,y=((v=t==null?void 0:t.cases)==null?void 0:v.length)??0,m=s.current>=s.total;R.innerHTML=`
    <div class="result-hero">
      <span class="result-hero__chip ${r.chipClass}">${r.chip}</span>
      <div class="result-hero__icon" aria-hidden="true">${r.icon}</div>
      <h1 class="result-hero__title">${O(r.title)}</h1>
      ...
      <div class="result-hero__meta">
        ...
        ${d==="correct"?`<span>·</span><span>테스트 케이스 <strong>${y}/${y}</strong> 통과</span>`:d==="wrong"?"<span>·</span><span>테스트 케이스 일부 실패</span>":""}
      </div>
    </div>
    ...
    <div class="result-compare">
      <div class="code-card">
        ...
        <pre class="code-card__body">${h(g)}</pre>
      </div>
      <div class="code-card">
        <div class="code-card__head">
          <span>참고 정답 코드</span>
        ...
        <pre class="code-card__body">${h(o)}</pre>
      </div>
    </div>
    ...
    <div class="result-encouragement">
      ...
      <p class="result-encouragement__text">
        <strong>${O(r.enc)}</strong>
      </p>
    </div>

    <div class="result-actions">
      <button type="button" class="result-back" id="exit-btn">테스트 종료하고 메인으로</button>
      ${m?'<button type="button" class="result-next result-next--final" id="next-btn">최종 결과 보기 →</button>':'<button type="button" class="result-next" id="next-btn">다음 문제 풀기 →</button>'}
    </div>
  `,document.getElementById("next-btn").addEventListener("click",()=>{if(m){V(s,a,n),window.scrollTo({top:0,behavior:"smooth"});return}const f={current:s.current+1,total:s.total};A(f);try{sessionStorage.removeItem(T)}catch{}C&&C.classList.remove("is-hidden"),setTimeout(()=>{window.location.href="test-gauge.html"},180)}),document.getElementById("exit-btn").addEventListener("click",z)}const P={correct:{label:"정답",icon:"✓",segClass:"summary-bar__seg--correct",cardClass:"summary-card--correct"},wrong:{label:"오답",icon:"✗",segClass:"summary-bar__seg--wrong",cardClass:"summary-card--wrong"},timeout:{label:"시간 초과",icon:"⏱",segClass:"summary-bar__seg--timeout",cardClass:"summary-card--timeout"},missing:{label:"미제출",icon:"—",segClass:"summary-bar__seg--missing",cardClass:"summary-card--missing"}};function I(s){const a=s==null?void 0:s.tag;if(!a)return(s==null?void 0:s.title)||`문제 ${s==null?void 0:s.id}`;const n=String(a).split("·").map(c=>c.trim()).filter(Boolean);return n[1]||n[0]||a}function U(s){const a=new Set,n=[];for(const{p:c,v:i}of s){if(i==="correct")continue;const t=I(c);a.has(t)||(a.add(t),n.push(t))}return n}function V(s,a,n){let c=0,i=0,t=0,d=0;const r=n.map((e,l)=>{const _=b.find(N=>N.id===e)??b[0],u=a[_.id],p=(u==null?void 0:u.verdict)||"missing";return p==="correct"?c++:p==="wrong"?i++:p==="timeout"?t++:d++,{p:_,v:p,a:u,slot:l+1}}),g=c,o=s.total,y=o?Math.round(c/o*100):0,m=g===o?"perfect":g>=Math.ceil(o*.6)?"good":"tryAgain",v={perfect:{title:"축하합니다 — 완벽해요!",sub:"5문제 모두 정답이에요. 코딩테스트 자신감을 얻으셨길 바랍니다."},good:{title:"잘 하셨어요!",sub:"좋은 페이스입니다. 부족한 부분은 정답 코드를 참고해 다듬어보세요."},tryAgain:{title:"여기서부터가 시작이에요.",sub:"오늘의 결과보다 더 중요한 건 흐름이에요. 한 번 더 도전해볼까요?"}},$=new Set(r.map(({p:e})=>I(e)).filter(Boolean)).size,Y=$>0?"도전한 개념 수":"도전한 문제 수",j=$>0?$:o,K=r.map(({slot:e,v:l})=>{const _=P[l];return`<span class="summary-bar__seg ${_.segClass}" title="문제 ${e} — ${_.label}"></span>`}).join(""),E=U(r);let w="";if(m==="perfect")w=`
    ...
    `;else if(m==="good"){const e=E.length?E.slice(0,3).map(l=>`<span class="summary-rec__chip">${h(l)}</span>`).join(""):"";w=`
      <div class="summary-rec__row">
        <span class="summary-rec__icon" aria-hidden="true">🎯</span>
        <div class="summary-rec__body">
    ...
    `}else{const e=E.length?E.slice(0,3).map(l=>`<span class="summary-rec__chip">${h(l)}</span>`).join(""):"";w=`
      <div class="summary-rec__row">
        <span class="summary-rec__icon" aria-hidden="true">📘</span>
        <div class="summary-rec__body">
    ...
    <div class="result-hero">
      ...
      <div class="summary-score">${g}<span class="summary-score__total"> / ${o}</span></div>
      <p class="summary-score__sub">정답 ${c} · 오답 ${i} · 시간 초과 ${t}${d?` · 미제출 ${d}`:""}</p>
      <h1 class="result-hero__title">${v[m].title}</h1>
      <p class="result-hero__sub">${v[m].sub}</p>
    </div>

    <div class="dash-section">
      <div class="dash-section__head">
        <h2 class="dash-section__h">전체 결과 한눈에 보기</h2>
        <span class="dash-section__pct">정답률 ${y}%</span>
      </div>
    ...
    <div class="dash-stats">
      <div class="dash-stat">
        <span class="dash-stat__num">${y}<span class="dash-stat__unit">%</span></span>
        <span class="dash-stat__label">정답률</span>
      </div>
      <div class="dash-stat">
        <span class="dash-stat__num">${c}<span class="dash-stat__unit">/${o}</span></span>
        <span class="dash-stat__label">맞힌 문제</span>
      </div>
      <div class="dash-stat">
        <span class="dash-stat__num">${j}</span>
        <span class="dash-stat__label">${Y}</span>
      </div>
      <div class="dash-stat">
    ...
    <div class="dash-section">
      <div class="dash-section__head">
        ...
        <span class="dash-section__pct">${o}문제</span>
      </div>
      <div class="summary-grid">
        ${r.map(({p:e,v:l,slot:_})=>{const u=P[l],p=I(e);return`
            <div class="summary-card ${u.cardClass}">
              ...
              <span class="summary-card__title">${h(e.title)}</span>
              <span class="summary-card__concept">${h(p)}</span>
  ...
  `,document.getElementById("restart-btn").addEventListener("click",()=>{if(confirm("진행 기록을 모두 지우고 처음부터 다시 시작할까요?")){for(const e of[x,M,T,B])try{sessionStorage.removeItem(e)}catch{}C&&C.classList.remove("is-hidden"),setTimeout(()=>{window.location.href="test-concepts.html"},180)}})}function z(){if(confirm("정말 테스트를 종료할까요? 진행 상황이 사라집니다.")){for(const s of[x,M,T,B])try{sessionStorage.removeItem(s)}catch{}window.location.href="index.html"}}const S=J(),G=Q(),L=H()??b.map(s=>s.id);S.total!==L.length&&(S.total=L.length,A(S));F(S,G,L);
