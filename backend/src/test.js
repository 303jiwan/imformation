...
testRouter.post("/progress", requireAuth, async (req, res, next) => {
  try {
    ...
    await stmts.upsertProgress.run(req.user.id, Math.floor(c), Math.floor(t));
...
testRouter.post("/answer", requireAuth, async (req, res, next) => {
  try {
    ...
    const info = await stmts.insertAnswer.run(req.user.id, problemId, code, v);
...
// GET /api/test/state
testRouter.get("/state", requireAuth, async (req, res, next) => {
  try {
    const progress = (await stmts.getProgress.get(req.user.id)) || {
      current: 0,
      total: 0,
      updated_at: null,
    };
    const answers = await stmts.listAnswers.all(req.user.id);
...
testRouter.post("/email-sample", async (req, res, next) => {
  try {
    ...
    await stmts.insertEmailSample.run(email.trim().toLowerCase(), userId);
    res.json({ ok: true });
  } catch (err) {
    next(err);
  }
