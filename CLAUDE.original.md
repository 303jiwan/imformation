...
Tech: full-stack app with a Vite multi-page frontend talking to an Express backend. Backend persistence is **Supabase Postgres** (via the `pg` driver, async) so accounts and per-user data sync across machines — every backend instance pointed at the same `DATABASE_URL` sees the same rows. C code submitted by users is executed via the **자체 채점기** (`backend/src/grader.js`): gcc 컴파일 1회 + stdin 갈아끼우며 N회 실행. 개발 환경은 `GRADER_UNSAFE_NOCONTAINER=1`(직접 실행), 프로덕션은 Docker 샌드박스(`gcc:9` 이미지)를 사용.
...
- [test-problems.js](frontend/src/test-problems.js) — hardcoded C-problem pool + queue helpers (`buildProblemQueue`, `loadProblemQueue`, `saveProblemQueue`, `getTestCases`). Each problem has an `expected(A)` JS reference that mocks what the user's C code should print. This is the single source of truth for problem content and per-case verdicts. If a real grading backend is wired in, this file is the only place that needs to change. **Dual purpose:** the problems here are not just test items — they will later be reused as the example/practice corpus for the C-syntax (문법) learning module. When adding new problems, treat each one as a teaching example for its `concepts` tag too: keep the `description` self-explanatory, write the `solution` so it reads like an idiomatic textbook snippet for that concept (not a clever one-liner), and prefer covering a missing concept over piling more onto an already well-covered one.
- [judge.js](frontend/src/judge.js) — 자체 채점기 클라이언트. `VITE_API_BASE`(기본 `http://localhost:3000`)로 백엔드 `/api/grade/*`를 호출. `runCMany`/`submitCMany` (배치 API) + `runC` (단일 케이스 호환 래퍼). 503/네트워크 오류 시 `_judgeAvailable=false` → 호출자가 mock 경로로 폴백.
